...
contract TestUSDC is Test {
    ...
    function testFuzz_transfer_have_balance_conserved(
        uint256 mintAmt,
        uint256 transferAmt
    ) public {
        // 1️⃣ Bound inputs to realistic values (avoid overflows / invalid fuzz cases)
        ...
        assertEq(
            token.balanceOf(alice),
            mintAmt - transferAmt,
            "Alice's balance should reduce correctly after transfer"
        );

        assertEq(
            token.balanceOf(aayush),
            transferAmt,
            "Aayush should have transfer amount"
        );
    }

    function testFuzz_transferFrom_respects_allowance_and_reduces_it(
        uint256 minted,
        uint256 allowanceAmt
    ) public {
        // Basic bounds to avoid pathological fuzz values
        ...
        assertEq(
            token.balanceOf(aayush),
            allowanceAmt,
            "aayush should have allowance amount"
        );
    }
}
